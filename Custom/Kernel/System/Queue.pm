...
sub GetAllQueues {
    ...
    my $MultiTenancy =
        $Kernel::OM->Get('Kernel::Config')->Get('MultiTenancy');
    ...
    if ( $Param{CustomerUserID} ) {
        ...
        # get group ids for customer user
    ...
    # fetch queues
    if ( $Param{UserID} && $Param{CustomerUserID} && $MultiTenancy ) {

        # check common groups of both, agent and customer user
        @GroupIDs = grep { $GroupList{$_} } @GroupIDs;
        $GroupString = join ', ', sort @GroupIDs;
        $CacheKey = "GetAllQueues::UserID::${Type}::${GroupString}::$Param{CustomerUserID}::$Param{UserID}";
        ...
        # check groups of agent user only
        $GroupString = join ', ', sort keys %GroupList;
        ...
        # check groups of customer user only
        $GroupString = join ', ', sort @GroupIDs;
