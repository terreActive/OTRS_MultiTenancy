...
This is an OTRS extension packages (OPM) providing dedicated resources for
groups of agents and customers.

MultiTenenacy makes that an agent user can see a customer user if (and only if)
they both are members of the same group. Any group will do. Best is to create
your own groups.
...
companies (for customer users). E.g. I have a role for each internal tickets,
...
If you have to control customer users's access to tickets, use services. There
is a customer users to service relation. It does not use groups, unfortunately.
